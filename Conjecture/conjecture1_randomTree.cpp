...
Graph generate_random_tree_network(int N, double alpha, double beta, bool selfLoops)
{
...
Graph initTree(int N)
{
    Graph g;
    //We randomly generate a Prufer Sequence:

    //Initialize Prufer sequence
    std::vector<int> PruferSeq(N-2,0);
    ...
    //std::cout << "PruferSeq: (" ;
    for(int n=0; n<N-2; n++){
        PruferSeq[n] =  distr(gen); // generate random numbers
        //std::cout << PruferSeq[n] << ",";
    }
    //std::cout << ")" << std::endl;


    //Transform Prufer sequence into the corresponding
    // adjacency matrix of the corresponding tree graph.

    // Collect the occurrences of every node in Prufer sequence in a vector:

    //Initialisation
    std::vector<int> v_occ(N,0);

    // Collect occurences from Prufer sequence
    for (int i=0;i<N-2;i++)
            v_occ[PruferSeq[i]-1]+=1;

    // We cycle through Prufer seq to find minimum node which is not in Prufer seq. for N-1 times:
    for (int i=0;i<N-2;i++){
            for (int j=0;j<N;j++){
            // If j+1 is not present in Prufer seq, we found our minimum.
                if (v_occ[j]==0){
                    // Remove from Prufer seq and add edge.
                    v_occ[j]=-1;
                    add_edge(j,PruferSeq[i]-1,g);
                    v_occ[PruferSeq[i]-1]--;
                    break;
                }
            }
    ...
    for (int i=0;i<N;i++){
        ...
        else if(v_occ[i]==0&&j==1){
            j_temp = i;
        }

    }
    add_edge(i_temp,j_temp,g);
    return g;
}


std::vector<double> randomtree_conjecture_1(int N, double threshold)
{
    Graph g = initTree(N);
...
int main(int argc, char *argv[])
{
    ...
    for(int j=start;j<end;j++){  
        ...
        std::vector result = randomtree_conjecture_1(N, threshold);
        for(int i=0;i<3;i++){
